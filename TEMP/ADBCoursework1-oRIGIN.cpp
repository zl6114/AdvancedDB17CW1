...
std::vector<std::string> findHours(odb::database& db, std::string username) {
	...
	//http://scm.codesynthesis.com/?p=odb/odb-examples.git;a=blob;f=inverse/driver.cxx;h=5a613e158ab809ca88bb79ead74c3137bad7c580;hb=HEAD
	//The first is to find the opening hours of all restaurants a user has rated
	shared_ptr<user> stl (db.load<user> (username));
	reviews& rvs (stl-> reviews());

//store review id to a vector
	for (reviews::iterator i (rvs.begin()); i != rvs.end(); ++ i)
	{
		lazy_weak_ptr<review>& lwp (*i);
		shared_ptr<review> p (lwp.load<review> ());
		cout << p->id << endl;
	}

//use stored review id to find all business
//stored all business id in a vector
//use stored business id to find the opening hours
//push opening hours to result
